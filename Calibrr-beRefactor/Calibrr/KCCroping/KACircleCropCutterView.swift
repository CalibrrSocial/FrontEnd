...
class KACircleCropCutterView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let circle = UIBezierPath(ovalIn: CGRect(x: rect.size.width/2 - 320/2, y: rect.size.height/2 - 320/2, width: 320, height: 320))
        context?.setBlendMode(.clear)
        UIColor.clear.setFill()
        circle.fill()
        
        //This is the same rect as the UIScrollView size 320 * 320, remains centered
       // let square = UIBezierPath(rect: CGRect(x: rect.size.width/2 - 320/2, y: rect.size.height/2 - 320/2, width: 320, height: 320))
