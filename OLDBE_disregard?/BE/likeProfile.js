...
exports.handler = async (event) => {
    ...
    return getUser(profileLikedId).then(
        function(data){
            return addLike(profileLikedId, data.Item.likes, userId)
        }
    ).then(
        function(data){
...
function addLike(profileLikedId, likes, userId)
{
    if (likes){
        var newLikes = likes
        ...
        newLikes.append(userId)
        return updateLikes(profileLikedId, newLikes)
    }
    return updateLikes(profileLikedId, [userId])
}
